...
class MeetingStatus(str, enum.Enum):
    ...
    processing = "processing"  # 转写完成，LangGraph 工作流处理中（任务提取、关联分析、报告）
    processed = "processed"    # 工作流处理完成
    failed = "failed"
...
class Meeting(Base):
    ...
    report_path: Mapped[str | None] = mapped_column(String(512), nullable=True)  # HTML 报告路径
